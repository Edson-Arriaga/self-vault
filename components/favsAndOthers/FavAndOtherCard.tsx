...
export default function FavAndOtherCard({fao, i, setIsEntryDetailsModalActive, setIsDeleteModalActive, setActiveFavAndOtherId} : FavAndOtherCardProps) {
  ...
  async function deleteFavAndOtherHandler(){
    ...
    setTimeout(() => {
      ...
      setFavAndOtherDeletedRef(cardRef)
    }, 50)
  }
  ...
  return (
    <Animatable.View animation={'bounceIn'} delay={i * 200} className='flex-row max-w-xl mx-auto' ref={cardRef}>
      <Pressable 
      ...
      >
        <View className="border-4 border-coral/50 p-1 flex-row items-center justify-between rounded-md">
          <Text className="flex-1 border-l-2 text-gray border-aqua pl-1 text-xl font-primary-semibold line-clamp-1 mt-2 mb-1 pt-2">
            {fao.entry}
          </Text>
